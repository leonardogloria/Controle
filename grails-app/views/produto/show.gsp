	...
	<body>
		...
		<div id="show-produto" class="content scaffold-show" role="main">
			...
			<ol class="property-list produto">
				...
				<g:if test="${produtoInstance?.fabricante}">
				<li class="fieldcontain">
					<span id="fabricante-label" class="property-label"><g:message code="produto.fabricante.label" default="Fabricante" /></span>
					
						<span class="property-value" aria-labelledby="fabricante-label"><g:link controller="fabricante" action="show" id="${produtoInstance?.fabricante?.id}">${produtoInstance?.fabricante?.nome.encodeAsHTML()}</g:link></span>
